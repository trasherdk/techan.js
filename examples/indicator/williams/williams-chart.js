function williamsChart (data, meta) {
  ...
  const williams = techan.plot.williams()
    .xScale(x)
    .yScale(y)

  const accessor = williams.accessor()
  ...
  return function (selection) {
    ...
    const williamsData = techan.indicator.williams()(sorted)
    x.domain(williamsData.map(accessor.d))
    y.domain(techan.scale.plot.williams().domain())

    svg.select('g.williams').datum(williamsData).call(williams)
