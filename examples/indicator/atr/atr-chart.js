function atrChart (data, meta) {
  ...
  const atr = techan.plot.atr()
    .xScale(x)
    .yScale(y)

  const accessor = atr.accessor()
  ...
  return function (selection) {
    ...
    const atrData = techan.indicator.atr()(sorted)
    x.domain(atrData.map(accessor.d))
    y.domain(techan.scale.plot.atr(atrData).domain())

    svg.select('g.atr').datum(atrData).call(atr)
