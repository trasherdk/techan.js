function aroonChart (data, meta) {
  ...
  const aroon = techan.plot.aroon()
    .xScale(x)
    .yScale(y)

  const accessor = aroon.accessor()
  ...
  return function (selection) {
    ...
    const aroonData = techan.indicator.aroon()(sorted)
    x.domain(aroonData.map(accessor.d))
    y.domain(techan.scale.plot.aroon().domain())

    svg.select('g.aroon').datum(aroonData).call(aroon)
